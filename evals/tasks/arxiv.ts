...
export const arxiv: EvalFunction = async ({
  ...
  try {
    await navengine.page.goto("https://arxiv.org/search/");

    await navengine.page.act(
      "search for papers about web agents with multimodal models",
    );

    const paper_links = await navengine.page.extract({
    ...
    for (const paper of paper_links.papers) {
      if (paper.link) {
        await navengine.page.goto(paper.link);
        const abstract = await navengine.page.extract({
