...
export class NavEngineActHandler {
  ...
  constructor({
    ...
    NavEngineContext: NavEngineContext;
  ...
  private async _verifyActionCompletion({
    ...
    await this.NavEnginePage._waitForSettledDom(domSettleTimeoutMs);
    ...
    const { outputString: domElements } =
      await this.NavEnginePage.page.evaluate(() => {
  ...
  private async _performPlaywrightMethod(
    ...
    const locator = this.NavEnginePage.page.locator(`xpath=${xpath}`).first();
    const initialUrl = this.NavEnginePage.page.url();
    ...
    if (method === "scrollIntoView") {
      ...
      try {
        ...
        for (const char of text) {
          await this.NavEnginePage.page.keyboard.type(char, {
      ...
      try {
        ...
        await this.NavEnginePage.page.keyboard.press(key);
      } catch (e) {
        this.logger({
          category: "action",
      ...
      this.logger({
        ...
        auxiliary: {
          url: {
            value: this.NavEnginePage.page.url(),
            type: "string",
          },
        },
      });
      ...
      if (method === "click") {
        ...
        const newOpenedTab = await Promise.race([
          new Promise<Page | null>((resolve) => {
            ...
            this.NavEnginePage.context.once("page", (page) => resolve(page));
        ...
        if (newOpenedTab) {
          ...
          await this.NavEnginePage.page.goto(newOpenedTab.url());
          await this.NavEnginePage.page.waitForLoadState("domcontentloaded");
          await this.NavEnginePage._waitForSettledDom(domSettleTimeoutMs);
        }

        await Promise.race([
          this.NavEnginePage.page.waitForLoadState("networkidle"),
        ...
        if (this.NavEnginePage.page.url() !== initialUrl) {
          this.logger({
            category: "action",
  ...
  private async getElement(
    ...
    try {
      const element = this.NavEnginePage.page.locator(`xpath=${xpath}`).first();
  ...
  private async _runCachedActionIfAvailable({
    ...
    const cacheObj = {
      url: this.NavEnginePage.page.url(),
      action,
      previousSelectors,
      requestId,
    };
    ...
    try {
      ...
      try {
        ...
        for (const xp of xpaths) {
          const candidate = this.NavEnginePage.page
            .locator(`xpath=${xp}`)
            .first();
          try {
        ...
        const originalUrl = this.NavEnginePage.page.url();
        ...
        if (this.NavEnginePage.page.url() !== initialUrl) {
          steps += `  Result (Important): Page URL changed from ${initialUrl} to ${this.NavEnginePage.page.url()}\n\n`;
        }
