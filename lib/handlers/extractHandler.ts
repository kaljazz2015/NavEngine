...
/**
 * The `NavEngineExtractHandler` class is responsible for extracting structured data from a webpage.
...
export class NavEngineExtractHandler {
  ...
  private async textExtract<T extends z.AnyZodObject>({
    ...
    const originalDOM = await this.NavEnginePage.page.evaluate(() =>
      window.storeDOM(),
    );

    // **3:** Process the DOM to generate a selector map of candidate elements
    const { selectorMap }: { selectorMap: Record<number, string[]> } =
      await this.NavEngine.page.evaluate(() => window.processAllOfDom());
    ...
    await this.NavEngine.page.evaluate(() => window.createTextBoundingBoxes());
    const pageWidth = await this.NavEngine.page.evaluate(
      () => window.innerWidth,
    );
    const pageHeight = await this.NavEngine.page.evaluate(
      () => window.innerHeight,
    );
  ...
  private async domExtract<T extends z.AnyZodObject>({
    ...
    const { outputString, chunk, chunks } = await this.NavEngine.page.evaluate(
